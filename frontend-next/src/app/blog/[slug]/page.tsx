...
const POSTS: Record<string, BlogPost> = {
  'seo-2026': {
    ...
    description: 'Descubre las tendencias SEO más importantes para 2026: GEO, AI Search, Core Web Vitals, E-E-A-T y estrategias de posicionamiento web para dominar los resultados de búsqueda.',
    date: '2026-01-15',
    readTime: 12,
    category: 'SEO',
    ...
    content: `<h2 id="la-era-de-geo">La Era de GEO</h2>
...
<p>La clave del éxito en <strong>optimización SEO</strong> este año es la integración: GEO + SEO técnico + contenido de calidad + link building estratégico + automatización inteligente. No se trata de elegir una u otra, sino de combinarlas en una estrategia cohesiva que genere resultados medibles y sostenibles.</p>`,
  ...
  'technical-seo': {
    ...
    description: 'Todo sobre SEO técnico: Core Web Vitals, schema markup, rendering, arquitectura de URLs, JavaScript SEO y herramientas esenciales para desarrolladores web.',
    date: '2025-12-01',
    readTime: 15,
    ...
    content: `<h2 id="que-es-el-seo-tecnico">¿Qué es el SEO Técnico?</h2>
...
<p>La implementación en Next.js es especialmente elegante: puedes crear componentes reutilizables que inyectan el JSON-LD en el head de cada página. Esto mantiene el código limpio y facilita el mantenimiento. Todo buen <strong>desarrollador web</strong> debería incluir schemas como parte estándar de su flujo de trabajo.</p>
...
<p>1. Core Web Vitals en verde (LCP &lt; 2.5s, INP &lt; 200ms, CLS &lt; 0.1)<br/>2. HTTPS implementado correctamente con certificado válido<br/>3. Sitemap XML actualizado y enviado a Search Console<br/>4. Robots.txt configurado correctamente<br/>5. Schema Markup validado sin errores<br/>6. URLs canónicas implementadas en todas las páginas<br/>7. Redirecciones 301 para URLs legacy<br/>8. Imágenes optimizadas con formatos modernos (WebP, AVIF)<br/>9. Mobile-first design responsive<br/>10. SSR o SSG para contenido indexable</p>
<p>Como <strong>desarrollador web</strong> especializado en SEO, recomiendo ejecutar esta checklist antes de cada lanzamiento y periódicamente para sitios en producción.</p>`,
  ...
  'nextjs-guide': {
    ...
    description: 'Guía práctica de Next.js 15 con App Router, Server Components, optimización SEO, API Routes con NestJS y estrategias de deployment para programadores profesionales.',
    date: '2025-11-15',
    readTime: 14,
    ...
    content: `<h2 id="por-que-nextjs">¿Por Qué Next.js?</h2>
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [...post.keywords, post.category.toLowerCase(), 'seo', 'desarrollo web', 'marketing digital', 'josé gaspard'],
    ...
    openGraph: {
      ...
      authors: [SITE.author],
      tags: post.tags,
      images: [{ url: `${SITE.url}/img/og-default.jpg`, width: 1200, height: 630, alt: post.title }],
    },
    twitter: {
        ...
        <section className="hero" style={{ minHeight: '40vh', paddingBottom: 0 }}>
          ...
          <div className="container hero-content" style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
              <span className="tag" style={{ background: '#3b82f6', color: '#fff', padding: '4px 14px', borderRadius: 20, fontSize: '0.8rem', fontWeight: 600 }}>
                {post.category}
              </span>
        ...
        <section className="section" style={{ paddingTop: 40 }}>
          <div className="container blog-layout" style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            <article style={{ minWidth: 0 }}>
              <div className="blog-article-content" style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', lineHeight: 1.85 }} dangerouslySetInnerHTML={{ __html: post.content }} />

              <div className="cta-banner" style={{ margin: '48px 0', padding: 32, background: 'linear-gradient(135deg, rgba(59,130,246,0.1) 0%, rgba(245,158,11,0.1) 100%)', borderRadius: 'var(--radius-lg)', border: '1px solid rgba(59,130,246,0.2)', textAlign: 'center' }}>
                <h3 style={{ marginBottom: 8, fontSize: '1.2rem' }}>¿Necesitas ayuda con {post.category}?</h3>
                <p style={{ color: 'var(--text-muted)', marginBottom: 20, fontSize: '0.95rem' }}>Agenda una consulta gratuita y analicemos cómo mejorar tu proyecto.</p>
                <Link href="/contact/" className="btn" style={{ background: '#f59e0b', color: '#000', fontWeight: 700, padding: '12px 32px', borderRadius: 'var(--radius-md)', textDecoration: 'none', display: 'inline-block' }}>
                  Contactar &rarr;
                </Link>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 40, marginBottom: 40 }}>
                {post.tags.map((tag) => (
                  <span key={tag} style={{ padding: '6px 14px', fontSize: '0.8rem', background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 20, color: 'var(--text-muted)' }}>
                    {tag}
                  </span>
                ))}
              </div>

              <div style={{ display: 'flex', gap: 20, padding: 32, background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-subtle)', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <p style={{ color: '#3b82f6', fontSize: '0.9rem', margin: '0 0 12px' }}>Arquitecto SEO &amp; Full-Stack Developer</p>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6, margin: '0 0 16px' }}>Experto SEO y desarrollador web con +15 años de experiencia. He trabajado con Google, Canva y PayPal optimizando el posicionamiento web y desarrollando soluciones full-stack escalables.</p>
                  <Link href="/contact/" className="btn" style={{ background: '#3b82f6', color: '#fff', padding: '10px 24px', borderRadius: 'var(--radius-md)', textDecoration: 'none', display: 'inline-block', fontSize: '0.9rem', fontWeight: 600 }}>
                    Trabajemos Juntos
                  </Link>
                </div>
              </div>
            ...
            <aside className="blog-sidebar">
              <div style={{ position: 'sticky', top: 100, display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div className="sidebar-widget" style={{ padding: 24, background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-subtle)' }}>
                  ...
                  <ul className="toc-list" style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                    {post.headings.map((h) => (
                      <li key={h.id} style={{ marginBottom: 8 }}>
                        <a href={`#${h.id}`} style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem', lineHeight: 1.5, display: 'block', padding: '4px 0 4px 12px', borderLeft: '2px solid var(--border-subtle)', transition: 'all 0.2s' }}>
                          {h.text}
                        </a>
                      </li>
                    ))}
                ...
                <div className="sidebar-widget" style={{ padding: 24, background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-subtle)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {relatedSlugs.map((rs) => (
                      <Link key={rs} href={`/blog/${rs}/`} style={{ display: 'block', padding: '12px 14px', background: 'var(--bg-card)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)', textDecoration: 'none', transition: 'border-color 0.2s' }}>
                        <span style={{ fontSize: '0.8rem', color: '#3b82f6', fontWeight: 600 }}>{POSTS[rs].category}</span>
                        <p style={{ margin: '4px 0 0', fontSize: '0.9rem', color: 'var(--text-primary)', lineHeight: 1.4 }}>{POSTS[rs].title}</p>
                      </Link>
                    ))}
                  </div>
                </div>

                <div className="sidebar-widget" style={{ padding: 24, borderRadius: 'var(--radius-lg)', background: 'linear-gradient(135deg, rgba(59,130,246,0.08) 0%, rgba(245,158,11,0.08) 100%)', border: '1px solid rgba(59,130,246,0.2)' }}>
                  <h3 style={{ fontSize: '1.05rem', marginBottom: 8 }}>¿Necesitas SEO profesional?</h3>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 16, lineHeight: 1.5 }}>Más de 15 años de experiencia en SEO técnico, link building y desarrollo web. Resultados medibles desde el primer mes.</p>
                  <Link href="/services/" className="btn" style={{ display: 'block', textAlign: 'center', background: '#f59e0b', color: '#000', fontWeight: 700, padding: '12px 0', borderRadius: 'var(--radius-md)', textDecoration: 'none', fontSize: '0.9rem' }}>
                    Ver Servicios &rarr;
                  </Link>
                </div>
              </div>
